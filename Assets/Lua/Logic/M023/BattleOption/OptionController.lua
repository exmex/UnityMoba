...
function OptionController:Awake(this)
  ...
  self.baseOption = self.leftPanel:Find("BaseOption")
  self.controlOption = self.leftPanel:Find("ControlOption")
  self.audioOption = self.leftPanel:Find("AudioOption")
  self.baseOptionPanel = self.this.transforms[1]
  ...
  --角色描边开关
  self.roleOutLineBar = self.baseOptionPanel:Find("RoleOutLine/Base/Bar")
  self.roleOutLineBase = self.baseOptionPanel:Find("RoleOutLine/Base")
  
  --帧数显示开关
  self.showFPSBar = self.baseOptionPanel:Find("ShowFps/Base/Bar")
  self.showFPSBase = self.baseOptionPanel:Find("ShowFps/Base")
  ...
  --局内打字开关
  self.gameInputBase = self.baseOptionPanel:Find("GameInput/Base")
  self.gameInputBar = self.gameInputBase:Find("Bar")

  --目标信息
  self.targetInfoBase = self.baseOptionPanel:Find("TargetInfo/Base")
  self.targetInfoBar = self.targetInfoBase:Find("Bar")
  ...
  --摄像机高度
  self.cameraHeightBase = self.baseOptionPanel:Find("CameraHeight/Base")
  self.cameraHeightBar = self.cameraHeightBase:Find("Bar")
  
  --摄像机移动
  self.cameraMoveBase = self.baseOptionPanel:Find("CameraMove/Base")
  self.cameraMoveBar = self.cameraMoveBase:Find("Bar")
  self.ll = self.cameraMoveBase:Find("ll")
  self.mm = self.cameraMoveBase:Find("mm")
  self.rr = self.cameraMoveBase:Find("rr")
  ...
  --摄像机敏感度
  self.cameraSenBar = self.baseOptionPanel:Find("CameraSensitivity/Base/Panel/Bar")
  self.cameraSenValue = self.baseOptionPanel:Find("CameraSensitivity/Base")
  
  --退出按钮
  self.exitButton = self.baseOptionPanel:Find("Exit/ExitButton")
  
  --投降按钮
  self.surrender = self.baseOptionPanel:Find("Exit/SurrenderButton")
  self.countDownText = self.surrender:Find("SurrenderCount")
  ...
  --技能取消方式
  self.cancelSkill1 = self.controlOptionPanel:Find("DiskUseSkill/CancelSkill/Selected")
  self.cancelSkill2 = self.controlOptionPanel:Find("DiskUseSkill/CancelSkill/Selected2")
  ...
  self.helpNoticeButton2 = self.controlOptionPanel:Find("DiskUseSkill/CancelSkill/HelpNoticeButton")
  ...
  local exitGame = function(self, e)
    self:DestroySelf()
  end
  listener.onPointerClick = NTGEventTriggerProxy.PointerEventDelegateSelf(exitGame, self)
  
  listener = NTGEventTriggerProxy.Get(self.helpNoticeButton1.gameObject)
  local helpNotice = function(self, e)
    self:HelpNoticeMoveCamera()
  end
  listener.onPointerClick = NTGEventTriggerProxy.PointerEventDelegateSelf(helpNotice, self)
...
function OptionController:TabControl(tabNum)
  if tabNum == 1 then
    --print("UTGDataOperator.Instance.WhereToUse " .. UTGDataOperator.Instance.WhereToUse)
    if UTGDataOperator.Instance.WhereToUse == 1 then
      ...
      self.baseOptionPanel.gameObject:SetActive(true)
...
function OptionController:RoleOutLine(delegate)
   local ob = self.roleOutLineBar:Find("Image"):GetComponent("NTGLuaScript").self
   ob.delegate = delegate
   ob.status[1] = UTGDataOperator.Instance.RoleOutLine 
end

function OptionController:ShowFPS(delegate)  
   local ob = self.showFPSBar:Find("Image"):GetComponent("NTGLuaScript").self
   ob.delegate = delegate
   ob.status[1] = UTGDataOperator.Instance.ShowFPS 
end

function OptionController:GameInput(delegate)
   local ob = self.gameInputBar:Find("Image"):GetComponent("NTGLuaScript").self
   ob.delegate = delegate
   ob.status[1] = UTGDataOperator.Instance.GameInput
end

function OptionController:CameraHeight(delegate)
   local ob = self.cameraHeightBar:Find("Image"):GetComponent("NTGLuaScript").self
   ob.delegate = delegate
   ob.status[1] = UTGDataOperator.Instance.CameraHeight  
end

function OptionController:TargetInfo(delegate)
   local ob = self.targetInfoBar:Find("Image"):GetComponent("NTGLuaScript").self
   ob.delegate = delegate
   ob.status[1] = UTGDataOperator.Instance.TargetInfo  
end

function OptionController:CameraMove(delegate)
   local ob = self.cameraMoveBar:Find("Image"):GetComponent("NTGLuaScript").self
   ob.delegate = delegate
   ob.status[1] = UTGDataOperator.Instance.CameraMove
   ob.ll = "关"
   ob.mm = "遥感"
   ob.rr = "滑动" 
end

function OptionController:DoCameraSiensitivityOnValueChanged(delegate)
  if delegate ~= nil then
    table.insert(self.coroutines,coroutine.start(OptionController.CameraSiensitivityOnValueChanged,self,delegate))
  end
end

function OptionController:CameraSiensitivityOnValueChanged(delegate)
  local preNum = 0
  while (true) do
    if preNum ~= self.cameraSenValue:GetComponent("UnityEngine.UI.Scrollbar").value then
      preNum = self.cameraSenValue:GetComponent("UnityEngine.UI.Scrollbar").value
      UTGDataOperator.Instance.CameraSensitivity = preNum
...
function OptionController:Test2()
  --print("abcbacbacbac")
end

function OptionController:ExitLogic(delegate)
  if UTGDataOperator.Instance.GameType == 0 then
    self.this.InvokeDelegate(delegate)
  elseif UTGDataOperator.Instance.GameType == 1 then
    self:NeedSHPanel("多人游戏不能退出")
  end
end

function OptionController:SurrenderLogic(delegate)
  self.SurrenderDelegate = delegate
end

function OptionController:CountDown(num)
  -- body
  self.countDownText:GetComponent(Text).text = "(" .. num .. ")"
  if (360 - tonumber(num)) > 0 then
    self.countDownText.gameObject:SetActive(true)
  else 
    self.countDownText.gameObject:SetActive(false)
    self.surrender:Find("Text"):GetComponent(Text).text = "发起投降"
    local listener = NTGEventTriggerProxy.Get(self.surrenderButton.gameObject)
    local surrender = function(self,e)
      self.this:InvokeDelegate(self.SurrenderDelegate)
    end
    listener.onPointerClick = NTGEventTriggerProxy.PointerEventDelegateSelf(surrender, self)
  end
end

function OptionController:CountControl()
  -- body
  if UTGDataOperator.Instance.GameType == 0 then
    self.surrender.gameObject:SetActive(false)
  elseif UTGDataOperator.Instance.GameType == 1 then
    self.surrender.gameObject:SetActive(true)
  end
end
--************基础设置结束
...
function OptionController:DoDiskS(delegate)
  if delegate ~= nil then
    table.insert(self.coroutines,coroutine.start(OptionController.DiskS,self,delegate))
  end
end
...
function OptionController:Shock(delegate)
   ...
   ob.delegate = delegate
   ob.status[1] = UTGDataOperator.Instance.Shock    
end
...
function OptionController:DoAudioVolumn(delegate)
  if delegate ~= nil then
    table.insert(self.coroutines,coroutine.start(OptionController.AudioVolumn,start,delegate))
  end
end
...
function OptionController:HelpNoticeMoveCamera()
  self.noticeMoveCamera.gameObject:SetActive(true)
  local listener = NTGEventTriggerProxy.Get(self.noticeMoveCamera:Find("Frame/IKnow").gameObject)
  local showHide = function(self, e)
    self.noticeMoveCamera.gameObject:SetActive(false)
  end
